...
void LoadCss(void) {
  ...
  gtk_css_provider_load_from_file(css_provider,
                                  g_file_new_for_path("interface.css"), &error);
...
static void MainMenuButton(GtkWidget *widget, gpointer data) {
  ...
  GtkWidget *openfile_child =
      gtk_stack_get_child_by_name(GTK_STACK(stack), "writespace_open");
  if (openfile_child != NULL) {
    gtk_container_remove(GTK_CONTAINER(stack), openfile_child);
  }
  GtkWidget *newfile_child =
      gtk_stack_get_child_by_name(GTK_STACK(stack), "writespace_newfile");
...
static void SetNewEntryName(GtkStyleContext *style, GtkWidget *window,
  ...
  g_signal_connect(save->window_popup, "delete-event", G_CALLBACK(PopupClose),
                   stack);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(ButtonClickedSetNewEntryName),
                   save);
...
static void Settings(GtkWidget *parent_window, GtkStyleContext *style) {
  ...
  gtk_window_set_transient_for(GTK_WINDOW(window_popup),
                               GTK_WINDOW(parent_window));
  gtk_window_set_position(GTK_WINDOW(window_popup),
                          GTK_WIN_POS_CENTER_ON_PARENT);
...
static void ButtonClickedNewEntry(GtkWidget *widget, gpointer data) {
  ...
  GtkWidget *write_space_page_new =
      WriteSpace("", mainmenu->style, mainmenu->window, mainmenu->stack);
  gtk_stack_add_named(GTK_STACK(mainmenu->stack), write_space_page_new,
                      "writespace_newfile");
  gtk_widget_show(write_space_page_new);
  gtk_stack_set_visible_child_name(GTK_STACK(mainmenu->stack),
                                   "writespace_newfile");
}
...
static void ButtonClickedOpenEntry(GtkWidget *widget, gpointer data) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      "choose an entry", GTK_WINDOW(mainmenu->window),
  ...
  GtkWidget *write_space_page_open =
      WriteSpace(filename, mainmenu->style, mainmenu->window, mainmenu->stack);
  gtk_stack_add_named(GTK_STACK(mainmenu->stack), write_space_page_open,
                      "writespace_open");
  gtk_widget_show(write_space_page_open);
  gtk_stack_set_visible_child(GTK_STACK(mainmenu->stack),
                              write_space_page_open);
...
GtkWidget *MainMenu(GtkWidget *stack, GtkWidget *window,
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(ButtonClickedNewEntry),
                   mainmenu);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(ButtonClickedOpenEntry),
                   mainmenu);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(ButtonClickedSettings),
                   mainmenu);
